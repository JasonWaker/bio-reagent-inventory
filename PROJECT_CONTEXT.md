...
`resolveBatch()` 按以下优先级寻找唯一库存批次：

1. 批号 `batchNo` 完全一致且只匹配到一个批次。
2. 出库 `productCode` 与库存 `sku` 一致且只匹配到一个批次。
3. 产品名称一致且只匹配到一个批次。
4. 否则标记为未匹配。
...
### 2026-09-17 02:20 — 功能：新增「退出登录」（未提交、未发生产，待用户确认）
- 类型：前端功能（本地已完成并验证，改动仅 `src/App.tsx`、`src/styles.css`）
...
- 双端验证（本地 dev + mock API，未触碰生产）：桌面 1280 宽 6 项全过（登录→侧栏退出确认/取消→确认退出 token 为 null→设置面板退出）；移动端流程全过（抽屉退出、设置退出、底部导航仍五项含“设置”、scrollWidth=innerWidth 无横向滚动）。测试浏览器无法强制 390 px（实际 814 px），新 CSS 均为 100% 宽度/flex，无固定宽度，风险低；建议发布后在真机 390/375/320 px 复验。
- 同步规则：用户明确新增强制规则——任何功能变化必须 PC/移动双端绝对同步，已写入第 14 节准则 10。
- 待办：经用户确认后提交并推送 `main`（GitHub Pages 自动发布，纯前端不涉 ECS/RDS）；回滚方式为 revert 该提交。

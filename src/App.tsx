...
import type { AppState, InventoryBatch, InventoryCycle, OutboundRecord } from './types'
...
export default function App() {
  ...
  const lowStock = batchViews.filter((row) => row.remaining <= state.lowStockThreshold)
  ...
  const importOutbound = async (file?: File) => {
    ...
    try {
      ...
      updateCycle((current) => ({ ...current, outbounds: [...current.outbounds, ...fresh] }))
      setView('outbound')
      setToast({ kind: 'success', message: `新增 ${fresh.length} 条出库记录，跳过 ${rows.length - fresh.length} 条重复记录` })
  ...
  return (
    <div className="app-shell">
        ...
        <div className="content">
          {view === 'dashboard' && (!cycle ? emptyPanel : <>
            ...
            <section className="metric-grid">
              ...
              <Metric label="低库存批次" value={`${lowStock.length} 个`} note={`阈值 ≤ ${state.lowStockThreshold} 盒`} tone="amber" icon={AlertTriangle} />
            ...
            <section className="dashboard-grid">
              ...
              <div className="panel"><div className="panel-heading"><div><span className="eyebrow">库存分布</span><h2>余量最低批次</h2></div></div>
                <div className="stock-bars">{[...batchViews].sort((a, b) => a.remaining - b.remaining).slice(0, 7).map((row) => { const max = Math.max(...batchViews.map((x) => Math.max(x.remaining, 0)), 1); return <div className="stock-bar" key={row.id}><div><strong>{row.name.replace(/测定试剂盒|检测试剂盒/, '')}</strong><span>{row.batchNo}</span></div><div className="bar-track"><i style={{ width: `${Math.max(3, Math.min(100, row.remaining / max * 100))}%` }} /></div><b className={row.remaining <= state.lowStockThreshold ? 'warning-text' : ''}>{formatNumber(row.remaining)}</b></div> })}</div>
              </div>
            </section>
          </>)}
          ...
          {view === 'settings' && <><section className="page-heading compact"><div><span className="eyebrow">浏览器本地模式</span><h1>数据设置</h1><p>设置预警阈值，并定期下载备份文件。</p></div></section><div className="settings-grid"><section className="panel settings-panel"><h2>预警规则</h2><label>低库存阈值（盒）<input type="number" min="0" value={state.lowStockThreshold} onChange={(e) => setState({ ...state, lowStockThreshold: Number(e.target.value) })} /></label><label>近效期提醒（天）<input type="number" min="1" value={state.expiryWarningDays} onChange={(e) => setState({ ...state, expiryWarningDays: Number(e.target.value) })} /></label></section><section className="panel settings-panel"><h2>备份与恢复</h2><p>GitHub Pages 不保存业务数据。更换设备或清理浏览器前，请先下载 JSON 备份。</p><div className="button-stack"><button className="button secondary" onClick={() => downloadJson(state)}><Download size={17} />下载完整备份</button><button className="button ghost" onClick={() => restoreInput.current?.click()}><Upload size={17} />恢复 JSON 备份</button></div></section><section className="panel settings-panel danger-zone"><h2>清除数据</h2><p>清除当前浏览器中的全部库存周期与设置。此操作无法撤销。</p><button className="button danger" onClick={() => { if (window.confirm('确定清除全部本地数据吗？此操作无法撤销。')) setState(EMPTY_STATE) }}><Trash2 size={17} />清除全部数据</button></section></div></>}
        </div>
      </main>
      ...
      {batchModal !== undefined && <BatchModal initial={batchModal} onClose={() => setBatchModal(undefined)} onSave={saveBatch} onDelete={batchModal ? () => { updateCycle((current) => ({ ...current, batches: current.batches.filter((row) => row.id !== batchModal.id) })); setBatchModal(undefined) } : undefined} />}
